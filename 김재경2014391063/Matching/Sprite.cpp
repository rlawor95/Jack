...
Sprite::Sprite(Bitmap* pBitmap, RECT& rcBounds, BOUNDSACTION baBoundAction)
{
	...
	m_baBoundsAction = baBoundAction;
	m_bHidden = FALSE;
}
...
SPRITEACTION Sprite::Update()
{
	...
	if (m_baBoundsAction == BA_WRAP) //WRAP상태일때
	{
	...
	else if (m_baBoundsAction == BA_BOUNCE)	//BOUNCE상태일때
	{
		BOOL bBounce = FALSE;
